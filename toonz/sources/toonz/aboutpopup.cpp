#include "ztorycbuild.h"  // commit and preview flag of this build
...
AboutPopup::AboutPopup(QWidget* parent)
  ...
  name += "\nBuilt: " __DATE__;
  // The commit, so a report can be traced to the exact build (previews above
...
#if ZTORYC_BUILD_PREVIEW
  QLabel* previewLabel = new QLabel(
      tr("PREVIEW BUILD — for testing, not for real work.\n"
         "Use the latest release for production."),
      this);
  previewLabel->setAlignment(Qt::AlignHCenter);
  previewLabel->setStyleSheet("color:#ff6a00;font-weight:bold;");
