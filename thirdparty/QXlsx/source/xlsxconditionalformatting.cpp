...
bool ConditionalFormatting::saveToXml(QXmlStreamWriter &writer) const
{
    ...
    for (int i = 0; i < d->cfRules.size(); ++i) {
        ...
        // Ztoryc patch: rules added via the public API never get a priority
        // assigned (it is only set when reading a file), so every rule was
        // written with priority="0".  Excel/LibreOffice require unique
        // priorities and silently keep only one rule otherwise.  Emit a unique
        // 1-based priority per rule within this block.
        writer.writeAttribute(QStringLiteral("priority"), QString::number(i + 1));
