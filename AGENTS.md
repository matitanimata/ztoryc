...
3. PR candidates (fixes developed in Ztoryc, clean enough to contribute upstream):
   ...
   **🔴 Da verificare su Tahoma2D — bug fix ad alto impatto:**
   - [ ] **CRASH cliccando la maniglia "Drawing #" dell'Animate tool** (`tnztools/edittool.cpp` → `DragDrawingNumberTool`, `toonzlib/stageobjectutil.cpp` `TStageObjectValues`, `toonzlib/txsheet.cpp` `addUndoDrawingNumberChange`). Cliccando il braccio Drawing Number del gizmo Animate l'app crasha **immediatamente** — sia nel main xsheet (nessun livello reale) sia dentro una scena su un livello normale. **VERIFICATO su Tahoma2D stock (2026-06-21)** → bug upstream confermato, NON regressione Ztoryc. Feature introdotta dalla PR upstream #2124 (`fix_animate_toolbar_and_pegbar_column`, merge `67f0ef7f4`). Crash log macOS: `EXC_BREAKPOINT`/`SIGTRAP` in `libsystem_malloc` (trap a indirizzo fisso `0x180d8d990` = corruzione heap / free invalido), stack mal-simbolicato (in overlay GL / Qt compose). ⚠️ Da diagnosticare con **lldb sul debug build** (`../debug-build`, repro reale + MallocScribble/guardmalloc per il free invalido) — i crash log non bastano. Sospetti: `TStageObjectValues::updateValues()` fa `getParam(T_DrawingNumber)` senza null-check (riga ~74); oppure il path undo `addUndoDrawingNumberChange`. Candidato PR upstream ad alto impatto (affligge tutti gli utenti).
