...
void Worksheet::saveToXmlFile(QIODevice *device) const
{
    ...
    // Ztoryc: auto-filter must appear right after <sheetData> per the schema.
    if (d->autoFilterRange.isValid()) {
        writer.writeStartElement(QStringLiteral("autoFilter"));
        writer.writeAttribute(QStringLiteral("ref"), d->autoFilterRange.toString());
